...
Cette fonction permet de créer une structure interne à nos données, par exemple des individus regroupés par zone. Les fonctions exécutées après l'appel à la fonction `group_by()` tiendront compte de cette structure interne. L'utilisation de la fonction `group_by()` associée à d'autres fonctions est très utile pour calculer des statistiques pour différents sites, traitements, espèces, ...
...
Cette permet d'agréger des lignes en appliquant une fonction sur une ou plusieurs colonnes :
...
Il existe différents manières de joindre des données, selon ce qu'on souhaite faire des données qui ne sont pas partagées.
...
Nous allons utiliser la fonction `left_join()` pour garder toutes les données présentes dans `croissance2` et y ajouter les données présentes dans `traitements` : 
