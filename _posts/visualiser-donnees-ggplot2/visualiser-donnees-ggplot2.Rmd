...
Nous pouvons remarquer que les espèces sont placées par ordre alphabétique le long de l'axe y. Nous préférerions que l'ordre respecte le nombre de pingouins par espèce. Nous pourrions pour cela définir des niveaux manuellement dans notre jeu de données à l'aide de fonctions de `dplyr` : 

```{r, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
# Trier les données
pingouins_tri <- pingouins %>% 
  mutate(espece = factor(espece, levels = c("Chinstrap", "Gentoo", "Adelie")))
...
Une autre façon de procéder tire profit du **pipe `%>%`** que nous avons vu dans le tutoriel sur le Tidyverse.
En partant du jeu de données d'origine, nous allons affecter à la variable "espece" des niveaux par ordre décroissant de nombre d'individus, puis enchaîner avec la création du barplot.

Cette façon de faire évite de multiplier les objets intermédiaires et d'encombrer son environnement de travail avec des objets avec des noms à rallonge (`pingouins_tri_taille_ailes_ordre_croissant`) ou des noms peu explicites (`test1`, `test_final`).

<br>

Le package `forcats` (qui fait lui aussi partie du `Tidyverse`) permet d'effectuer des opérations sur des facteurs. Nous allons ainsi demander à R de transformer la variable "espece" en facteur, avec des niveaux qui respectent l'ordre allant de l'espèce ayant le plus grand nombre de pingouins à celle ayant le plus faible nombre de pingouins.

La fonction `fct_infreq` permet de faire cela, associée à la fonction `mutate()` du package `dplyr` qui permet de transformer une colonne. Il existe d'autres fonctions similaires, comme `fct_inseq()` qui utilise pour définir les niveaux d'un facteur l'ordre d'apparition des éléments dans la variable.

```{r, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
# Utilisation du pipe pour trier les données puis générer le barplot
pingouins %>% 
...
Les espèces sont bien triées, mais dans le mauvais ordre ! Pour corriger cela, il suffit d'ajouter la fonction `fct_rev()` à la fonction `fct_infreq()` : 

```{r, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
# Inverser l'odre des barres
pingouins %>% 
...
Nous souhaitons étudier le **rapport entre la longueur du bec et la longueur des ailes**, mais au lieu de représenter les trois espèces sur un seul graphique, nous voudrions avoir un graphique par espèce. Nous pourrions le faire en générant un graphique par espèce à l'aide de la fonction `filter(espece == ...)` et d'un pipe `%>%`, et en répétant l'opération trois fois, une pour chaque espèce. Ou nous pouvons utiliser la fonction `facet_wrap()` qui permet de **séparer nos données en fonction de la variable espèce**.

Commençons par placer les données des trois espèces sur un seul graphique : 

```{r, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
# Rapport entre la longueur du bec et la longueur des ailes - 1 seul graphique pour les trois espèces
ggplot(data = pingouins,
       mapping = aes(x = aile_lngr_mm,
                     y = bec_lngr_mm)) +
  geom_point(aes(colour = espece),
