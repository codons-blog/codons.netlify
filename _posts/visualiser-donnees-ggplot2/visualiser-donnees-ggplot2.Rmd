...
Nous aimerions utiliser d'autres polices de caractères pour notre graphiques. 
...
Le package `patchwork` permet de combiner plusieurs graphiques en une seule image.
...
Nous précisons ensuite, à l'aide de `plot_layout()` que nous voulons une taille relative plus importante pour la deuxième ligne (`heights = c(1, 2)`). Le code `guides = "collect"` indique que les légendes des différents graphiques sont à regrouper (très utile pour éviter d'avoir plusieurs fois la même légende dans le graphique final).
