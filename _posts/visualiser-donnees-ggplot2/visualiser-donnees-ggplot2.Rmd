...
(p1 <- p +
  theme(panel.background = element_rect(fill = "#1d3557", colour = NA),
        panel.border = element_rect(fill = NA, colour = "white", size = 3),
...
## Utiliser des couleurs dans le titre ou le sous-titre

Si peu de légendes sont nécessaires à la bonne compréhension du graphique, il est possible d'alléger celui-ci en utilisant du texte coloré dans le titre ou le sous-titre pour remplacer la légende.

Nous utilisons pour cela le package `ggtext` : 

```{r, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
# Installer et charger le package ggtext
...
Nous allons modifier le sous-titre du graphique, à l'aide de la syntaxe **markdown**. Pour colorer le texte "Adelie" en orange, cela donne le code suivant : 

```{r, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
<span style= 'color:darkorange;'>**Adelie**</span>
```

Nous indiquons ainsi que le mot "Adelie" est à colorer en orange (`color:darkorange`) et à mettre en gras (`**Adelie**`).

Voici comment modifier le sous-titre et les autres paramètres (taille et position du titre et du sous-titre, marges et suppression de la légende) : 
...
p +
  ...
  theme(plot.title = element_text(size = 25, hjust = 0.5),
        plot.subtitle = element_markdown(size = 20, hjust = 0.5, margin = margin(t = 5, b = 15)),
        plot.title.position = "plot",
